...
```
clientId: https://solidcommunity.au/apps/resume_writer/client-profile.jsonld
```
...
- **If you maintain solidcommunity.au** (the ANU Solid Community), deploy these
  to `apps/resume_writer/` alongside the other apps (this is how `filepod` is set
  up).
...
```bash
curl -I https://solidcommunity.au/apps/resume_writer/client-profile.jsonld   # expect 200
```
...
`com.example.resumewriter://redirect` custom scheme.
